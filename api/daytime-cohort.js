...
const START=new Date("2026-09-15T14:15:00Z");
const END=new Date("2026-09-15T17:00:00Z");
const BATCH_AFTER=new Date("2026-09-15T12:25:00Z");
const TARGET=120;
const STEP=30;
...
export default async function handler(req,res){try{
 const sql=db();
 const cohort=await sql`select fa_flight_id,scheduled_out_initial from flight_jobs where created_at>=${BATCH_AFTER.toISOString()}::timestamptz and scheduled_out_initial>=${START.toISOString()}::timestamptz and scheduled_out_initial<${END.toISOString()}::timestamptz and origin like 'K%' and destination like 'K%'`;
 if(cohort.length>=TARGET)return json(res,200,{ok:true,done:true,batch_total:cohort.length,target:TARGET,window:[START,END],min_lead_minutes:MIN_LEAD_MIN,prediction:"DAYTIME_MAE_CLOSER_TO_5.25_THAN_6.51",bin_counts:Object.entries(cohort.reduce((a,r)=>(a[binOf(r.scheduled_out_initial)]=(a[binOf(r.scheduled_out_initial)]||0)+1,a),{}))});
 const now=Date.now();
 ...
 const seen=new Map();
 for(const f of raw){const o=code(f.origin),d=code(f.destination),sm=new Date(f.scheduled_out).getTime();if(!f.fa_flight_id||!f.scheduled_out||!o.startsWith("K")||!d.startsWith("K")||sm<scanStart.getTime()||sm>=END.getTime()||/cancel/i.test(String(f.status||"")))continue;if(!seen.has(f.fa_flight_id))seen.set(f.fa_flight_id,f);}
 ...
 const after=await sql`select scheduled_out_initial from flight_jobs where created_at>=${BATCH_AFTER.toISOString()}::timestamptz and scheduled_out_initial>=${START.toISOString()}::timestamptz and scheduled_out_initial<${END.toISOString()}::timestamptz and origin like 'K%' and destination like 'K%' order by scheduled_out_initial`;
 const bins={};for(const r of after){const b=binOf(r.scheduled_out_initial);bins[b]=(bins[b]||0)+1;}
 return json(res,200,{ok:true,done:after.length>=TARGET,registered_now:done.length,batch_total:after.length,target:TARGET,new_candidates:candidates.length,failed:failed.length,airport_errors,window:[START,END],min_lead_minutes:MIN_LEAD_MIN,prediction:"DAYTIME_MAE_CLOSER_TO_5.25_THAN_6.51",bin_counts:bins});
